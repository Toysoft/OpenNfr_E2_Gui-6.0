...
MTV_Version = "Medizin-TV v0.90"

MTV_siteEncoding = 'utf-8'

class MedizinTvGenreScreen(MPScreen):
	...
	def layoutFinished(self):
		self.genreliste.append(('Aktuelles Programm', 1, 'http://www.medizin-tv.tv.grid-tv.com/c/mid,2798,aktuelles_Programm/?contentpart=prog_video'))
		self.genreliste.append(('Filme auf Abruf', 2, 'http://www.medizin-tv.tv.grid-tv.com/c/mid,2802,Filme_auf_Abruf/'))
		self.genreliste.append(('TV-Programmvorschau', 3, 'http://www.medizin-tv.tv.grid-tv.com/c/mid,2800,TV-Programmvorschau/'))
	...
	def keyOK(self):
		...
		if genreID == 1:
			self.session.open(
				GermanyTVPlayer2,
				[(genre, tvLink)],
				'Medizin-TV - aktuelles Programm'
				)
		else:
			self.session.open(MedizinTvListScreen, genreID, tvLink, genre)

class MedizinTvListScreen(MPScreen):
	...
	def __init__(self, session, genreID, tvLink, stvGenre):
		...
		self['F1'] = Label(_("Exit"))
		self.keyLocked = True
		self.baseUrl = "http://www.medizin-tv.tv"
	...
	def keyOK(self):
		...
		if self.genreID == 2:
			self.session.open(
				GermanyTVPlayer,
				self.filmliste,
				"http://www.medizin-tv.tv.grid-tv.com/inc/mod/video/play.php/vid,%s/q,mp4/hq,1/typ,ondemand/file.mp4",
