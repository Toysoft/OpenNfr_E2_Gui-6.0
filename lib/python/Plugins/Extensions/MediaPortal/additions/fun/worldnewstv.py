...
WNTV_Version = "WORLD NEWS-TV v0.90"

WNTV_siteEncoding = 'utf-8'

class WorldnewsTvGenreScreen(MPScreen):
	...
	def layoutFinished(self):
		self.genreliste.append(('Aktuelles Programm', 1, 'http://www.worldnews-tv.tv.grid-tv.com/?contentpart=prog_video'))
		self.genreliste.append(('Filme auf Abruf', 2, 'http://www.worldnews-tv.tv.grid-tv.com/c/mid,1425,Filme_auf_Abruf/'))
		self.genreliste.append(('TV-Programmvorschau', 3, 'http://www.worldnews-tv.tv.grid-tv.com/c/mid,4843,TV-Programmvorschau/'))
	...
	def keyOK(self):
		...
		if genreID == 1:
			self.session.open(
				GermanyTVPlayer2,
				[(genre, tvLink)],
				'WORLD NEWS-TV - aktuelles Programm'
				)
		else:
			self.session.open(WorldnewsTvListScreen, genreID, tvLink, genre)

class WorldnewsTvListScreen(MPScreen):
	...
	def __init__(self, session, genreID, stvLink, stvGenre):
		...
		self['F1'] = Label(_("Exit"))
		self.keyLocked = True
		self.baseUrl = "http://www.worldnews-tv.tv"
	...
	def layoutFinished(self):
		self.keyLocked = True
		twAgentGetPage(self.stvLink).addCallback(self.genreData).addErrback(self.dataError)
	...
	def keyOK(self):
		...
		if self.genreID == 2:
			self.session.open(
				GermanyTVPlayer,
				self.filmliste,
				"http://www.worldnews-tv.tv.grid-tv.com/inc/mod/video/play.php/vid,%s/q,mp4/hq,1/typ,ondemand/file.mp4",
