...
OTV_Version = "OPERA-TV v0.90"

OTV_siteEncoding = 'utf-8'

class OperaTvGenreScreen(MPScreen):
	...
	def layoutFinished(self):
		self.genreliste.append(('Aktuelles Programm', 1, 'http://www.opera-tv.tv.grid-tv.com/?contentpart=prog_video'))
		self.genreliste.append(('Filme auf Abruf', 2, 'http://www.opera-tv.tv.grid-tv.com/c/mid,2079,Filme_auf_Abruf/'))
		self.genreliste.append(('TV-Programmvorschau', 3, 'http://www.opera-tv.tv.grid-tv.com/c/mid,2594,TV-Programmvorschau/'))
	...
	def keyOK(self):
		...
		if genreID == 1:
			self.session.open(
				GermanyTVPlayer2,
				[(genre, tvLink)],
				'OPERA-TV - aktuelles Programm'
				)
		else:
			self.session.open(OperaTvListScreen, genreID, tvLink, genre)

class OperaTvListScreen(MPScreen):
	...
	def __init__(self, session, genreID, tvLink, stvGenre):
		...
		self['F1'] = Label(_("Exit"))
		self.keyLocked = True
		self.baseUrl = "http://www.opera-tv.com"
	...
	def keyOK(self):
		...
		if self.genreID == 2:
			self.session.open(
				GermanyTVPlayer,
				self.filmliste,
				"http://www.opera-tv.com/inc/mod/video/play.php/vid,%s/q,mp4/hq,1/typ,ondemand/file.mp4",
